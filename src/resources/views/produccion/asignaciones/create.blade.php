...
<div class="container mx-auto px-4 py-6">
    <div class="flex items-center gap-4 mb-6">
        <a href="{{ $proyecto_seleccionado ? route('produccion.proyectos.show', $proyecto_seleccionado) : route('produccion.asignaciones.index') }}" class="text-gray-500 hover:text-gray-700">← Volver</a>
    ...
    @if($empleados->isEmpty())
        <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            ...
            <a href="{{ $proyecto_seleccionado ? route('produccion.proyectos.show', $proyecto_seleccionado) : route('produccion.asignaciones.index') }}" class="text-indigo-600 hover:text-indigo-800 text-sm mt-2 inline-block">Volver al proyecto</a>
        </div>
    @else
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 max-w-xl">
            <form action="{{ route('produccion.asignaciones.store') }}" method="POST" class="p-6 space-y-5">
                @csrf
